...
def patch_archive_page_ux(repo: str, token: str, iso_date: str, site_path: str) -> bool:
    ...
    try:
        ...
        # -----------------------------
        # 2) Ensure loading badge exists (NO swipe hint)
        # -----------------------------
        nav_loading_block = """
      <div id="navLoading" class="navLoading" aria-live="polite" aria-atomic="true">
        <span class="badge">날짜 이동 중…</span>
...
"""
        if 'id="navLoading"' not in html_new:
            ...
            if got:
                s, e, nav_block = got
                html_new = html_new[:e] + nav_loading_block + html_new[e:]

        # Remove swipe hint block entirely (never show)
        html_new = re.sub(r'(?is)\s*<div\s+id="swipeHint"[^>]*>.*?</div>\s*', '\n', html_new)
        html_new = html_new.replace("좌우 스와이프로 날짜 이동", "")
