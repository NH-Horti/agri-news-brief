...
def is_relevant(title: str, desc: str, dom: str, url: str, section_conf: dict, press: str) -> bool:
    ...
    if normalize_host(dom).endswith("news1.kr") and ("/local/" in _path):
        ...
        has_apc = ("apc" in text) or ("산지유통" in text) or ("산지유통센터" in text)
...
def _headline_gate_relaxed(a: "Article", section_key: str) -> bool:
    """코어(핵심2) 부족 시 사용하는 완화 게이트.
    - 여전히 오피니언/행사성/인물 기사 등은 제외
    - 섹션별로 '핵심 맥락'은 유지하되, 엄격 게이트보다 약간 넓게 허용
    """
    title = (a.title or "").lower()
    text = (a.title + " " + a.description).lower()

    # 공통: 칼럼/기고/인물/행사성은 완화에서도 제외
    if has_any(title, [w.lower() for w in _HEADLINE_STOPWORDS]):
        return False

    if section_key == "supply":
        core_terms = ["가격", "시세", "수급", "작황", "생산", "출하", "물량", "재고", "저장", "물가", "할인", "경락", "반입", "도매"]
        crop_terms = [
            "사과", "배", "감귤", "만감", "한라봉", "레드향", "천혜향", "황금향",
            "포도", "샤인머스캣", "자두", "복숭아", "매실", "키위", "참다래", "유자",
            "딸기", "파프리카", "참외", "오이",
            "화훼", "절화", "국화", "장미",
        ]
        # 품목 맥락은 반드시 유지 + 가격/수급/유통 신호 중 1개 이상
        return has_any(text, crop_terms) and has_any(text, core_terms)

    if section_key == "policy":
        if _is_policy_official(a):
            return True
        action_terms = ["대책", "지원", "할인", "할당관세", "검역", "고시", "개정", "발표", "추진", "확대", "연장", "단속", "브리핑", "보도자료", "예산", "TF", "회의"]
        ctx_terms = ["농산물", "농식품", "농업", "원예", "과수", "과일", "채소", "화훼", "수급", "가격", "유통", "원산지", "도매시장", "가락시장", "수출", "수입", "검역", "통관"]
        return has_any(text, action_terms) and has_any(text, ctx_terms)

    if section_key == "dist":
        # dist는 완화해도 APC 단독 오탐을 막기 위해 '인프라/시장' 신호를 요구
        dist_terms = ["가락시장", "도매시장", "공판장", "경락", "경락가", "경매", "반입", "중도매인", "시장도매인", "선별", "ca저장", "저온", "저장", "물류", "수출", "검역", "통관", "원산지"]
        apc_terms = ["apc", "산지유통", "산지유통센터"]
        infra_terms = ["준공", "가동", "개장", "확충", "확대", "자동화", "스마트", "선별기", "저온창고", "물류센터", "시설", "장비", "라인"]
        if count_any(text, dist_terms) >= 1:
            return True
        if has_any(text, apc_terms) and has_any(text, infra_terms):
            return True
        return False
    ...
    if section_key == "pest":
        ...
        strict_hits = count_any(text, [t.lower() for t in PEST_STRICT_TERMS])
        weather_hits = count_any(text, [t.lower() for t in PEST_WEATHER_TERMS])
        # 완화: 둘 중 하나라도 1회 이상이면 후보 자격
        return (strict_hits >= 1) or (weather_hits >= 1)
