import os
import json
import re
from datetime import datetime, timedelta, time
from zoneinfo import ZoneInfo
from email.utils import parsedate_to_datetime
import requests
import holidays

KST = ZoneInfo("Asia/Seoul")
KR_HOLIDAYS = holidays.KR()

# -----------------------------
# 0) 운영 파라미터 (여기만 고치면 확장 쉬움)
# -----------------------------
ANCHOR_HOUR = 7  # 매일 "07:00 KST" 기준으로 창을 끊어서 집계
MAX_PER_QUERY = 100

SECTION_QUERIES = {
    "1) 품목 및 수급 동향": [
        "사과 저장량 가격", "배 도매가격", "단감 시세", "곶감 떫은감 탄저병",
        "한라봉 레드향 시세", "감귤 가격", "참다래 키위 가격",
        "샤인머스캣 가격", "풋고추 오이 시설채소 가격 일조량",
        "절화 졸업 입학 시즌 가격", "쌀 산지 가격 비축미 방출"
    ],
    "2) 주요 이슈 및 정책": [
        "농산물 온라인 도매시장 허위거래 이상거래", "농산물 할인 지원 연장",
        "할당관세 수입 과일 검역 완화", "가락시장 휴무 경매 재개"
    ],
    "3) 병해충 및 방제": [
        "과수화상병 약제 신청", "과수화상병 궤양 제거", "기계유유제 월동 해충 방제",
        "탄저병 예방", "과수 동해 냉해 대비"
    ],
    "4) 유통 및 현장(APC/수출 등)": [
        "농협 APC 스마트 AI 선별기 CA 저장", "공판장 도매시장 물량 동향",
        "농식품 수출 실적 배 딸기", "가락시장 경매 물량 동향"
    ]
}

# “원예수급부 관점에서 거리 먼 지역행사/복지성 기사” 대략 필터(원하면 수정)
IRRELEVANT_PATTERNS = [
    r"온누리상품권", r"환급", r"지역축제", r"바자회", r"기부", r"나눔행사"
]

# 카카오 메시지(짧게)
KAKAO_BUTTON_TITLE = "브리핑 열기"


# -----------------------------
# 1) 공통 유틸
# -----------------------------
def kst_now() -> datetime:
    return datetime.now(tz=KST)

def anchor_end_time(now: datetime) -> datetime:
    """오늘 07:00 KST 시각을 end로 고정 (실행이 07:03이든 07:20이든 end는 07:00으로)."""
    return datetime.combine(now.date(), time(ANCHOR_HOUR, 0), tzinfo=KST)

def is_business_day(d: datetime) -> bool:
    """월~금 + 공휴일 제외"""
    if d.weekday() >= 5:  # 5=Sat,6=Sun
        return False
    return d.date() not in KR_HOLIDAYS

def norm_title(t: str) -> str:
    t = re.sub(r"<.*?>", "", t)  # html tags
    t = re.sub(r"\s+", " ", t).strip()
    t = re.sub(r"[^\w\s가-힣]", "", t)
    return t.lower()

def looks_irrelevant(title: str) -> bool:
    for p in IRRELEVANT_PATTERNS:
        if re.search(p, title):
            return True
    return False


# -----------------------------
# 2) GitHub Gist (state + latest.md)
# -----------------------------
def gh_headers(token: str) -> dict:
    return {
        "Authorization": f"Bearer {token}",
        "Accept": "application/vnd.github+json",
        "User-Agent": "agri-news-brief-bot"
    }

def gist_get(gist_id: str, token: str) -> dict:
    r = requests.get(f"https://api.github.com/gists/{gist_id}", headers=gh_headers(token), timeout=30)
    r.raise_for_status()
    return r.json()

def gist_update_files(gist_id: str, token: str, files_payload: dict) -> dict:
    r = requests.patch(
        f"https://api.github.com/gists/{gist_id}",
        headers=gh_headers(token),
        json={"files": files_payload},
        timeout=30
    )
    r.raise_for_status()
    return r.json()


def load_state_from_gist(gist_json: dict) -> dict:
    files = gist_json.get("files", {})
    if "state.json" in files and "content" in files["state.json"]:
        try:
            return json.loads(files["state.json"]["content"] or "{}")
        except Exception:
            pass
    return {"last_end_kst": None}


# -----------------------------
# 3) Naver News Search API 수집
# -----------------------------
def naver_news_search(query: str, client_id: str, client_secret: str, display: int = 100) -> list[dict]:
    url = "https://openapi.naver.com/v1/search/news.json"
    headers = {
        "X-Naver-Client-Id": client_id,
        "X-Naver-Client-Secret": client_secret,
    }
    params = {
        "query": query,
        "display": min(display, 100),
        "start": 1,
        "sort": "date"
    }
    r = requests.get(url, headers=headers, params=params, timeout=30)
    r.raise_for_status()
    return r.json().get("items", [])


def parse_pubdate_to_kst(pub: str) -> datetime | None:
    # Naver pubDate: RFC822 형식이 많음
    try:
        dt = parsedate_to_datetime(pub)
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=ZoneInfo("UTC"))
        return dt.astimezone(KST)
    except Exception:
        return None


def collect_articles(start_kst: datetime, end_kst: datetime) -> dict[str, list[dict]]:
    nav_id = os.environ["NAVER_CLIENT_ID"]
    nav_secret = os.environ["NAVER_CLIENT_SECRET"]

    # 섹션별 기사 누적
    out: dict[str, list[dict]] = {k: [] for k in SECTION_QUERIES.keys()}

    seen_links = set()
    seen_titles = set()

    for section, queries in SECTION_QUERIES.items():
        for q in queries:
            items = naver_news_search(q, nav_id, nav_secret, display=MAX_PER_QUERY)
            for it in items:
                pub_kst = parse_pubdate_to_kst(it.get("pubDate", ""))
                if not pub_kst:
                    continue
                if not (start_kst <= pub_kst < end_kst):
                    continue

                title = it.get("title", "")
                clean_title = re.sub(r"<.*?>", "", title)
                if looks_irrelevant(clean_title):
                    continue

                link = it.get("originallink") or it.get("link") or ""
                if not link:
                    continue

                nt = norm_title(clean_title)
                if link in seen_links or nt in seen_titles:
                    continue

                seen_links.add(link)
                seen_titles.add(nt)

                out[section].append({
                    "title": clean_title.strip(),
                    "link": link,
                    "pub_kst": pub_kst.strftime("%Y-%m-%d %H:%M"),
                    "query": q
                })

    # 섹션별 최신순 정렬
    for section in out:
        out[section].sort(key=lambda x: x["pub_kst"], reverse=True)

    return out


# -----------------------------
# 4) OpenAI 요약(선택) - Responses API
# -----------------------------
def openai_summarize(articles_by_section: dict[str, list[dict]], start_kst: datetime, end_kst: datetime) -> str:
    api_key = os.getenv("OPENAI_API_KEY", "").strip()
    if not api_key:
        return simple_compose_md(articles_by_section, start_kst, end_kst, note="(OPENAI_API_KEY 없음: 제목 기반 나열)")

    model = os.getenv("OPENAI_MODEL", "gpt-5")

    # 입력(너무 길면 비용/실패 가능 → 링크/제목 중심)
    lines = []
    lines.append(f"기간: {start_kst:%Y-%m-%d %H:%M}~{end_kst:%Y-%m-%d %H:%M} (KST)")
    for sec, items in articles_by_section.items():
        lines.append(f"\n[{sec}]")
        if not items:
            lines.append("- (해당 없음)")
            continue
        for it in items[:25]:  # 과도하게 길어지는 것 방지
            lines.append(f"- {it['pub_kst']} | {it['title']} | {it['link']}")

    instructions = (
        "너는 농협중앙회 원예수급부(과수화훼) 팀장 결재용 '농산물 뉴스 브리핑' 작성자다.\n"
        "규칙:\n"
        "1) 한국어, plain text(굵게/이모지 과다 금지).\n"
        "2) 섹션 순서 유지.\n"
        "3) 각 기사: 요약 1줄(20~45자 권장) + 다음 줄에 링크.\n"
        "4) 중복 이슈는 통합하되 링크는 1개 이상 남겨라.\n"
        "5) 기사 없는 섹션은 '특이사항 없음' 한 줄.\n"
    )

    payload = {
        "model": model,
        "instructions": instructions,
        "input": "\n".join(lines)
    }
    r = requests.post(
        "https://api.openai.com/v1/responses",
        headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"},
        json=payload,
        timeout=60
    )
    r.raise_for_status()
    data = r.json()

    # Responses API는 output_text를 쉽게 추출
    text = ""
    for item in data.get("output", []):
        if item.get("type") == "message":
            for c in item.get("content", []):
                if c.get("type") == "output_text":
                    text += c.get("text", "")
    return text.strip() or simple_compose_md(articles_by_section, start_kst, end_kst, note="(OpenAI 응답 파싱 실패: 제목 기반 나열)")


def simple_compose_md(articles_by_section: dict[str, list[dict]], start_kst: datetime, end_kst: datetime, note: str = "") -> str:
    out = []
    out.append(f"[농산물 뉴스 Brief] {start_kst:%Y-%m-%d %H:%M}~{end_kst:%Y-%m-%d %H:%M} KST {note}".strip())
    for sec, items in articles_by_section.items():
        out.append(f"\n{sec}")
        if not items:
            out.append("특이사항 없음")
            continue
        for it in items:
            out.append(f"- {it['title']} ({it['pub_kst']})")
            out.append(it["link"])
    return "\n".join(out)


# -----------------------------
# 5) 카카오: refresh_token → access_token 갱신 → 나에게 보내기
# -----------------------------
def kakao_refresh_access_token() -> str:
    """
    Kakao token endpoint: https://kauth.kakao.com/oauth/token :contentReference[oaicite:6]{index=6}
    Client secret 기능이 기본 활성화된 경우 client_secret 파라미터 필요 :contentReference[oaicite:7]{index=7}
    """
    refresh_token = os.environ["KAKAO_REFRESH_TOKEN"].strip()
    client_id = os.environ["KAKAO_REST_API_KEY"].strip()
    client_secret = os.getenv("KAKAO_CLIENT_SECRET", "").strip()

    data = {
        "grant_type": "refresh_token",
        "client_id": client_id,
        "refresh_token": refresh_token,
    }
    if client_secret:
        data["client_secret"] = client_secret

    r = requests.post(
        "https://kauth.kakao.com/oauth/token",
        headers={"Content-Type": "application/x-www-form-urlencoded;charset=utf-8"},
        data=data,
        timeout=30
    )
    r.raise_for_status()
    j = r.json()
    return j["access_token"]


def kakao_send_to_me(access_token: str, text: str, link_url: str):
    """
    Kakao memo default send API: /v2/api/talk/memo/default/send :contentReference[oaicite:8]{index=8}
    메시지 길이 제한(대략 1000byte 수준) 때문에, 카톡에는 '요약+링크' 권장 :contentReference[oaicite:9]{index=9}
    """
    url = "https://kapi.kakao.com/v2/api/talk/memo/default/send"
    headers = {"Authorization": f"Bearer {access_token}"}

    template_object = {
        "object_type": "text",
        "text": text,
        "link": {"web_url": link_url, "mobile_web_url": link_url},
        "button_title": KAKAO_BUTTON_TITLE
    }

    data = {"template_object": json.dumps(template_object, ensure_ascii=False)}
    r = requests.post(url, headers=headers, data=data, timeout=30)
    r.raise_for_status()
    return r.json()


# -----------------------------
# 6) main
# -----------------------------
def main():
    # 필수 env 확인
    required = [
        "NAVER_CLIENT_ID", "NAVER_CLIENT_SECRET",
        "KAKAO_REST_API_KEY", "KAKAO_REFRESH_TOKEN",
        "GIST_ID", "GH_GIST_TOKEN"
    ]
    missing = [k for k in required if not os.getenv(k)]
    if missing:
        raise RuntimeError(f"Missing env: {missing}")

    now = kst_now()
    end_kst = anchor_end_time(now)

    # 오늘이 휴일/주말이면 "발송하지 않음" (다음 영업일에 누적)
    force = os.getenv("FORCE_SEND", "0") == "1"
    if (not force) and (not is_business_day(end_kst)):
        print(f"[SKIP] Not a business day (end_kst={end_kst:%Y-%m-%d}). No send, state not updated.")
        return

    gist_id = os.environ["GIST_ID"]
    gh_token = os.environ["GH_GIST_TOKEN"]

    gist_json = gist_get(gist_id, gh_token)
    state = load_state_from_gist(gist_json)

    # 마지막 end가 없으면 기본 24시간
    last_end_str = state.get("last_end_kst")
    if last_end_str:
        last_end_kst = datetime.fromisoformat(last_end_str).astimezone(KST)
    else:
        last_end_kst = end_kst - timedelta(hours=24)

    start_kst = last_end_kst

    print(f"[INFO] Window KST: {start_kst} ~ {end_kst}")

    # 수집
    articles_by_section = collect_articles(start_kst, end_kst)

    # 전문 생성 (OpenAI 있으면 요약 강화)
    brief_text = openai_summarize(articles_by_section, start_kst, end_kst)

    # gist에 latest.md 먼저 업로드(전문 확인용)
    updated = gist_update_files(
        gist_id, gh_token,
        {"latest.md": {"content": brief_text}}
    )
    gist_url = updated.get("html_url") or gist_json.get("html_url") or f"https://gist.github.com/{gist_id}"

    # 카톡용 짧은 메시지(너무 길면 실패/잘림 가능 → 짧게)
    # (섹션별 '기사 있는 경우만' 1줄씩 하이라이트)
    highlights = []
    for sec, items in articles_by_section.items():
        if items:
            highlights.append(f"- {sec}: {items[0]['title']}")
    if not highlights:
        highlights.append("- 주요 기사: 없음(기간 내 검색 결과 기준)")

    kakao_text = (
        f"농산물 뉴스 브리핑\n"
        f"{start_kst:%m/%d %H:%M}~{end_kst:%m/%d %H:%M} KST\n"
        f"{chr(10).join(highlights[:4])}\n"
        f"(전문은 버튼 클릭)"
    )

    # 카카오 전송
    access_token = kakao_refresh_access_token()
    kakao_send_to_me(access_token, kakao_text, gist_url)

    # 전송 성공 후 state 업데이트(성공했을 때만 창을 앞으로 당김)
    new_state = {"last_end_kst": end_kst.isoformat()}
    gist_update_files(
        gist_id, gh_token,
        {"state.json": {"content": json.dumps(new_state, ensure_ascii=False)}}
    )

    print("[OK] Sent to Kakao + Updated gist/state.")


if __name__ == "__main__":
    main()
